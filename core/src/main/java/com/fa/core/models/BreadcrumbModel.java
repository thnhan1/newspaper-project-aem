...
public class BreadcrumbModel implements Breadcrumb {
     ...
     * 2. First newspaper: tag found on the current page
     */
    ...
    /**
     * Builds breadcrumb items by finding section/subsection pages under the
     * language root whose cq:tags contain the given tag ID.
     *
     * Strategy: walk children of languageRoot (sections) and their children
     * (subsections) to find pages tagged with the resolved primary tag.
     * If the match is on a subsection page, add both section parent and subsection.
     * If on a section page, add only the section.
     */
    private void buildItemsFromTag(String tagId, Page languageRoot) {
        ...
        while (sections.hasNext()) {
            ...
            if (!showHidden && section.isHideInNav()) {
                continue;
            }

            if (pageHasTag(section, tagId)) {
                items.add(new BreadcrumbItem(section, false, linkManager, false));
                return;
            }
            ...
            while (subsections.hasNext()) {
                ...
                if (!showHidden && subsection.isHideInNav()) {
                    continue;
                }
                if (pageHasTag(subsection, tagId)) {
                    items.add(new BreadcrumbItem(section, false, linkManager, false));
                    items.add(new BreadcrumbItem(subsection, false, linkManager, false));
                    return;
                }
            }
        }
        LOG.debug("No section/subsection page found matching tag: {}", tagId);
    }

    private boolean pageHasTag(Page page, String tagId) {
